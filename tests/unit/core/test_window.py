...
def setup_pygame_for_tests(monkeypatch: pytest.MonkeyPatch) -> Generator[None, None, None]:
    ...
    class MockTransform:
        @staticmethod
        def scale(surface: Any, size: Tuple[int, int], dest_surface: Any = None) -> None:
            pass

    # Create mock display module
    class MockDisplay:
        @staticmethod
        def set_mode(size: Tuple[int, int], flags: int = 0, depth: int = 0, 
                    display: int = 0, vsync: int = 0) -> MockSurface:
...
def test_fullscreen_initialization() -> None:
    ...
    window = Window(config)
    # Just verify it initializes without error, can't test actual fullscreen in headless mode
...
def test_cleanup() -> None:
    ...
    window = Window(config)
    window.__del__()  # Call destructor directly instead of cleanup
