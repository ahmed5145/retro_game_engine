...
import pytest
import pygame
from src.core.sprite import Sprite, SpriteSheet, SpriteFrame, SpriteConfig

def create_test_image(width: int, height: int, color: tuple[int, int, int], path: str) -> None:
...
def test_sprite_sheet_add_frames_grid() -> None:
    ...
    try:
        ...
        assert sprite_sheet.frames[0].x == 0 and sprite_sheet.frames[0].y == 0  # Top-left
        assert sprite_sheet.frames[1].x == 32 and sprite_sheet.frames[1].y == 0  # Top-right
        assert sprite_sheet.frames[2].x == 0 and sprite_sheet.frames[2].y == 32  # Bottom-left
        assert sprite_sheet.frames[3].x == 32 and sprite_sheet.frames[3].y == 32  # Bottom-right
